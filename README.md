...
4. Copy name of the corresponding driver to the `Driver` property of the `[Asio.Output]`, `[Asio.Input.0]`, `[Asio.Input.1]` and `[Asio.Input.Mic]` blocks of the `RS_ASIO.ini`
...
- Some Focusrite devices have been reported to only output sound properly when using ASIO buffer sizes of 48, 96 or 192. Use the custom buffer size setting `CustomBufferSize` in `RS_ASIO.ini`'s `[Asio]` block for this, in addition to setting the same value in the Focusrite Settings.
